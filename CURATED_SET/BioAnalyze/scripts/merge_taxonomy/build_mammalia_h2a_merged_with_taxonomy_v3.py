"""
Build mammalia H2A merged dataset with taxonomy (v3).

v3 uses the same base pipeline as v2 and adds an explicit accession
enrichment step (NCBI protein -> gene -> Ensembl) for rows missing all IDs.
Re-running this script regenerates v3 deterministically from current sources.
"""
...
OUTPUT_CSV = MERGED_DIR / "mammalia_H2A_merged_with_taxonomy_v3.csv"
...
def fetch_ensembl_from_gene_id(gene_id: str, session: requests.Session, cache: Dict[str, str]) -> str:
    if gene_id in cache:
    ...
    try:
        ...
        for dbtag in root.findall(".//Dbtag"):
            db = norm(dbtag.findtext("Db")).lower()
            if db != "ensembl":
                continue
            obj = dbtag.find("./Object-id/Str")
            if obj is not None and obj.text:
                ensembl_id = obj.text.strip()
                break
    except Exception:
        ensembl_id = ""

    cache[gene_id] = ensembl_id
    time.sleep(0.34)
    return ensembl_id


def needs_accession_enrichment(row: Dict[str, str]) -> bool:
    return not (
        norm(row.get("vgnc_id"))
        or norm(row.get("hgnc_id"))
        or norm(row.get("ncbi_id"))
        or norm(row.get("ensembl_gene_id"))
    )
...
def enrich_accessions(
    ...
    ensembl_cache: Dict[str, str] = {}

    for row in rows:
        if not needs_accession_enrichment(row):
            continue
        ...
        # Avoid duplicate audit rows per accession.
        if accession in seen:
            # Still apply cached result to the row if available.
            protein_uid = protein_cache.get(accession, "")
            gene_id = gene_cache.get(protein_uid, "") if protein_uid else ""
            ensembl_id = ensembl_cache.get(gene_id, "") if gene_id else ""
            if gene_id and not norm(row.get("ncbi_id")):
                row["ncbi_id"] = gene_id
            if ensembl_id and not norm(row.get("ensembl_gene_id")):
        ...
        if not protein_uid:
            unresolved.append(
                {
                    "accession": accession,
                    "organism": norm(row.get("organism")),
                    "taxonomy id": norm(row.get("taxonomy id")),
                    "source": "ncbi",
                    "ncbi_id": "",
                    "ensembl_gene_id": "",
        ...
        gene_id = fetch_gene_id_from_protein_uid(protein_uid, session, gene_cache)
        if not gene_id:
            unresolved.append(
                {
                    "accession": accession,
        ...
        ensembl_id = fetch_ensembl_from_gene_id(gene_id, session, ensembl_cache)

        # Apply to all matching rows.
        for r in rows:
            if norm(r.get("accession")) != accession:
                continue
            if not norm(r.get("ncbi_id")):
                r["ncbi_id"] = gene_id
            if ensembl_id and not norm(r.get("ensembl_gene_id")):
                r["ensembl_gene_id"] = ensembl_id
...
def main() -> None:
    ...
    audit_accession_resolved: List[Dict[str, str]] = []
    audit_accession_unresolved: List[Dict[str, str]] = []
    ...
    with requests.Session() as session:
        ...
        # Step 0: accession enrichment for rows missing all IDs
        audit_accession_resolved, audit_accession_unresolved = enrich_accessions(base_rows, session)
    ...
    write_csv(
        AUDIT_DIR / "audit_accession_enrichment_resolved.csv",
        audit_accession_resolved,
        ["accession", "organism", "taxonomy id", "source", "ncbi_id", "ensembl_gene_id", "reason"],
    )
    write_csv(
        AUDIT_DIR / "audit_accession_enrichment_unresolved.csv",
        audit_accession_unresolved,
        ["accession", "organism", "taxonomy id", "source", "ncbi_id", "ensembl_gene_id", "reason"],
    )
    ...
    print(f"ACCESSION_ENRICHMENT_RESOLVED={len(audit_accession_resolved)}")
    print(f"ACCESSION_ENRICHMENT_UNRESOLVED={len(audit_accession_unresolved)}")

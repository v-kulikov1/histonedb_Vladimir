...
def normalized_entropy(codons: Sequence[str]) -> float:
    unique_count = len(set(codons))
    if unique_count <= 1:
        return 0.0
    return float(shannon_entropy(codons) / np.log2(unique_count))
...
def collect_position_codons(
    ...
    for position in positions:
        ...
        for order in valid_orders:
            ...
            row.append(normalized_entropy(codons))
            for codon in codons:
                aa = STANDARD_TABLE.forward_table.get(codon)
                if aa and aa != human_aa:
